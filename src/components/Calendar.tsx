  ...
  return (
    <div className="min-h-screen p-4 pb-24">
      <div className="max-w-md mx-auto space-y-6">
        ...
        >
          <h1 className="text-text">Progress Calendar</h1>
        ...
        >
          <Card className="bg-gradient-to-br from-blue to-mauve text-crust border-none">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-crust">
            ...
            <CardContent className="p-4">
              ...
              <div className="grid grid-cols-7 gap-2 mb-2">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                  <div key={day} className="text-center text-sm font-medium text-subtext0">
                    {day}
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-7 gap-2">
                {calendarDays.map((dayData, index) => {
                  ...
                  let bgColor = 'bg-surface0';
                  let icon = null;
                  let textColor = 'text-overlay2';

                  if (isAfterQuit && !isFuture) {
                    if (hasCigarettes) {
                      bgColor = 'bg-red/20 border-2 border-red';
                      textColor = 'text-red';
                      icon = <XCircle className="w-3 h-3 text-red" />;
                    } else {
                      bgColor = 'bg-green/20 border-2 border-green';
                  ...
                  if (isToday) {
                    bgColor = bgColor + ' ring-2 ring-blue';
                  }
        ...
        >
          <Card>
            <CardHeader>
              <CardTitle className="text-text">Legend</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-green/20 border-2 border-green flex items-center justify-center">
                  <CheckCircle2 className="w-4 h-4 text-green" />
                </div>
                <span className="text-sm text-subtext1">Smoke-free day</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-red/20 border-2 border-red flex items-center justify-center">
                  <XCircle className="w-4 h-4 text-red" />
                </div>
                <span className="text-sm text-subtext1">Day with slip-up(s)</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-surface0" />
                <span className="text-sm text-subtext1">Before quit date / Future</span>
              </div>
            </CardContent>
          </Card>
        </motion.div>
        ...
        {smokFreeDays > 0 && (
          ...
          >
            <Card className="bg-gradient-to-r from-green to-teal border-none">
              <CardContent className="p-6 text-center text-crust">
                <p className="text-lg">
